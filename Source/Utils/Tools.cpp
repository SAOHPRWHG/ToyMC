...
void load_png_texture(const char *file_name) {
    ...
    unsigned char *data = stbi_load(file_name, &width, &height, &comp, 4);
    //error = lodepng_decode32_file(&data, &width, &height, file_name);
    if (!data) {
        fprintf(stderr, "load_png_texture %s failed\n", file_name);
        exit(1);
    }
...
int char_width(char input) {
    ...
    return lookup[static_cast<int>(input)];
}
