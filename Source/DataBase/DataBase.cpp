...
int db_init(const char *path) {
    ...
    constexpr const char *create_query =
    ...
    constexpr const char *insert_block_query =
        "insert or replace into block (p, q, x, y, z, w) "
        "values (?, ?, ?, ?, ?, ?);";
    constexpr const char *insert_light_query =
        "insert or replace into light (p, q, x, y, z, w) "
        "values (?, ?, ?, ?, ?, ?);";
    constexpr const char *insert_sign_query =
        "insert or replace into sign (p, q, x, y, z, face, text) "
        "values (?, ?, ?, ?, ?, ?, ?);";
    constexpr const char *delete_sign_query =
        "delete from sign where x = ? and y = ? and z = ? and face = ?;";
    constexpr const char *delete_signs_query =
        "delete from sign where x = ? and y = ? and z = ?;";
    constexpr const char *load_blocks_query =
        "select x, y, z, w from block where p = ? and q = ?;";
    constexpr const char *load_lights_query =
        "select x, y, z, w from light where p = ? and q = ?;";
    constexpr const char *load_signs_query =
        "select x, y, z, face, text from sign where p = ? and q = ?;";
    constexpr const char *get_key_query =
        "select key from key where p = ? and q = ?;";
    constexpr const char *set_key_query =
